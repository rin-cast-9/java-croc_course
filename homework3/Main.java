        ...
        try (Scanner in = new Scanner(System.in)) { // so that we won't need to in.close()
            ...
            while (flag) {
                ...
                try { 
                    ...
                    System.err.printf("Invalid input. Try again...%n"); // provides a user with an appropriate message
                }
                ...
                int check = 0; // sets check for the while loop below to get correct input from a user as well as to catch an exception that might be thrown
                while (check == 0) {
                    try {
                        check = in.nextInt(); // reads input from a user
                        break; // if exception isn't thrown then breaks from the loop
                    }
                    catch (InputMismatchException exception) {
                        System.err.printf("Invalid input. Try again...%n"); // provides a user with an appropriate message
                        in.next(); // cleans the scanner 
                    }
                }
